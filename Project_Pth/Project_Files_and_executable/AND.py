...
def and_op(a = 0, b = 0) -> int:
    if(not (a.isnumeric()) or not (b.isnumeric())):
        return str("Invalid Input")
    return int(int(a) & int(b))

def and_show(a = 0, b = 0) -> int:
    if(not (a.isnumeric()) or not (b.isnumeric())):
        return str("Invalid Input")
    return str(bin(int(a) & int(b)))

def and_info(a = 0, b = 0):
    tr = Tk()
    a = str(a)
    b = str(b)
    ...
    Label(gridy, text = '   AND :       ', pady = '10', padx = '10', relief = 'ridge', fg = '#ADD8E9', font = ('helvetica', '25'), width = '15', height = '1').grid(column = 0, row = 2)
